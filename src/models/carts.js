...
exports.GetDetailItemCart = (id) => {
  return new Promise((resolve, reject) => {
    runQuery(`SELECT * FROM carts WHERE _id=${id}`, (err, results, fields) => {
      if (err) {
        return reject(new Error(err))
      }
...
exports.UpdateItemCart = (idCart, dataItem) => {
  return new Promise((resolve, reject) => {
    const { totalItem, totalPrice } = dataItem
    runQuery(`UPDATE carts SET total_items=${totalItem},total_price=${totalPrice} WHERE _id=${idCart}`,
      (err, results, fields) => {
        if (err) {
          return reject(new Error(err))
        }
